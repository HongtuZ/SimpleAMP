...
def amp_data_discriminator_obs(
    ...
    motion_seq_times = motion_data.sample_motion_seq_times(
        motion_ids, n_steps, env.step_dt
    )
    motion_seq_data = motion_data.get_motion_seq_data(
        motion_ids, motion_seq_times, asset.joint_names, asset.body_names
    )  # (num_envs, [n_bodies], n_steps, dim)

    yaw_quat = math_utils.yaw_quat(motion_seq_data["root_quat_w"])
    root_quat_b = math_utils.quat_mul(
        math_utils.quat_conjugate(yaw_quat), motion_seq_data["root_quat_w"]
    )  # shape: (num_envs, num_steps, 4)
    root_rotm_b = math_utils.matrix_from_quat(
        root_quat_b
    )  # (num_envs, num_steps, 3, 3)
