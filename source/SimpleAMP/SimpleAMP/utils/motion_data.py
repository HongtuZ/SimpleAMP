...
class MotionData():
    ...
    def _load_motion_data(self):
        ...
        for motion_name, motion_weight in self.motion_data_weights.items():
            ...
            if motion_name not in motion_name2path.keys():
                raise ValueError(f"Motion name {motion_name} defined in motion weights not found in motion data directory {str(motion_data_dir)}. Available names: {motion_name2path.keys()}")
            ...
            root_ang_vel_w[:-1] = math_utils.quat_apply(root_quat_w[:1], math_utils.quat_box_minus(root_quat_w[1:], root_quat_w[:-1])) / dt
    ...
    def sample_motion_times(self, motion_ids: torch.Tensor, truncate_time_start: float = None, truncate_time_end: float = None) -> torch.Tensor:
    ...
    def get_motion_data(self, motion_ids: torch.Tensor, motion_times: torch.Tensor, joint_names: list = None, body_names: list = None) -> torch.Tensor:
        ...
        if joint_names:
            joint2isaac_idxs = torch.tensor([self.joint_names.index(name) for name in joint_names], dtype=torch.long, device=self.device)
            joint_pos = joint_pos[:, joint2isaac_idxs]
            joint_vel = joint_vel[:, joint2isaac_idxs]
        if body_names:
            body2isaac_idxs = torch.tensor([self.body_names.index(name) for name in body_names], dtype=torch.long, device=self.device)
    ...
    def get_motion_seq_data(self, motion_ids: torch.Tensor, motion_seq_times: torch.Tensor, joint_names: list = None, body_names: list = None) -> dict[str, torch.Tensor]:
